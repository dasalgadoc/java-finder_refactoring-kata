...
public class LowestAgeComparor implements AgeComparorByCriteria{
    ...
    public PeopleTuple getRequestedPeopleTuple(List<PeopleTuple> peopleTuples) {
        return peopleTuples.stream().min((firstPerson,secondPerson) -> firstPerson.getBirthDateDifferenceInMilliseconds().compareTo(secondPerson.getBirthDateDifferenceInMilliseconds())).get();
    }
